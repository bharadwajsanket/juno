...
fun CircularEqControl(
    ...
    val lastTickValues = remember { floatArrayOf(mid, bass, treble) }
    ...
    fun dispatchToAxis(axisIdx: Int, value: Float) {
        if (axisIdx in 0..2) {
            val lastVal = lastTickValues[axisIdx]
            if (kotlin.math.abs(value - lastVal) >= 0.5f) {
                hapticManager.performMicroTick()
                lastTickValues[axisIdx] = value
            }
        }
        when (axisIdx) {
