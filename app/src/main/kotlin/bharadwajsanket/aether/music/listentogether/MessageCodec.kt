...
import bharadwajsanket.aether.music.listentogether.proto.Listentogether
...
class MessageCodec(
    ...
    private fun toProtoMessage(payload: Any): MessageLite {
        return when (payload) {
            ...
            is ChatPayload -> {
                ...
                payload.replyTo?.let { builder.setReplyTo(repliedMessageToProto(it)) }
                builder.build()
            }
        ...
        return when (msgType) {
            ...
            MessageTypes.CHAT -> {
                ...
                ChatMessagePayload(
                    ...
                    replyTo = if (pb.hasReplyTo()) protoToRepliedMessage(pb.replyTo) else null
                )
            }
            else -> null
        }
    ...
    private fun repliedMessageToProto(reply: RepliedMessage): Listentogether.RepliedMessage {
        return Listentogether.RepliedMessage.newBuilder()
            .setUsername(reply.username)
            .setMessage(reply.message)
            .build()
    }

    private fun protoToRepliedMessage(proto: Listentogether.RepliedMessage): RepliedMessage {
