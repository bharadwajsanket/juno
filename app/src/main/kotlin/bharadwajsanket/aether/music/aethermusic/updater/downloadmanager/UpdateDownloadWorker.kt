package bharadwajsanket.aether.music.aethermusic.updater.downloadmanager
...
class UpdateDownloadWorker(private val context: Context, workerParams: WorkerParameters) :
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val downloadFile = if (isZip) File(downloadDir, "echo_temp.zip") else File(downloadDir, "aethermusic.apk")
            ...
            val finalFile = if (isZip) {
                val targetApkFile = File(downloadDir, "aethermusic.apk")
                var extracted = false
                try {
